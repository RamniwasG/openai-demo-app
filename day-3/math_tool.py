...
@tool
def simple_math_tool(operand1: float, operand2: float, operation: Literal["add", "subtract"]) -> str:
    """
        Perform simple addition or subtraction on two numbers.
        Specify 'add' or 'subtract' for the operation.
    """
    result = 0
    if operation == 'add':
        result = operand1 + operand2
    elif operand2 == 'subtract':
        result = operand2 - operand1
    else:
        return "Invalid operation specified, Use 'add' or 'subtract' "

    return f"result of {operand1} and {operand2} with {operation} operation is : {result}"


tools_list_po = [simple_math_tool]
app_po = build_graph_one_tool(tools_list_po)
print("\n Graph build for Math operation.")

query = input("Enter your query please! \n ")

history, output = app_call(app_po, query)
